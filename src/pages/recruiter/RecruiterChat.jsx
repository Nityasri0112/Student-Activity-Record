...
export default function RecruiterChat() {
  ...
  if (!user) {
    return <div className="bg-white p-6 rounded-lg">Please login to access chat</div>;
  }
        ...
        <div className="space-y-2">
          {students.map(s => (
            <button
              ...
              className={`w-full text-left p-3 rounded ${selected?.id === s.id ? 'bg-brand text-white' : 'bg-gray-50 hover:bg-gray-100'}`}
            >
              {s.name}
            </button>
          ))}
